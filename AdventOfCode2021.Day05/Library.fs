    ...
    let calc lineFilter (input: string seq) =
        ...
        let lines =
            ...
            |> Seq.map (fun regexMatch -> {
                Start = (regexMatch.X1.Value, regexMatch.Y1.Value) |> Tuple.map int
                End = (regexMatch.X2.Value, regexMatch.Y2.Value) |> Tuple.map int
            })
            |> Seq.filter lineFilter
            |> Seq.toList

        let allPoints = lines |> Seq.map expandLine |> Seq.concat |> Seq.toList

        let countBy = allPoints |> Seq.countBy id

        countBy |> Seq.filter (fun (_, count) -> count > 1) |> Seq.map snd |> Seq.length
